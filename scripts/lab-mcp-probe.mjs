...
export class McpProbe {
 ...
 this.child.stdin.on('error',e=>this.fail(e));this.child.on('error',e=>this.fail(e));this.child.on('exit',(code,signal)=>{this.exit={code,signal};this.fail(new Error('MCP process exited '+code+' '+signal));});
 }
 ...
 async close(){this.save();if(this.exit!==undefined)return;this.child.stdin.end();await new Promise((resolve,reject)=>{const killTimer=setTimeout(()=>{if(this.exit===undefined)this.child.kill();},1500),limit=setTimeout(()=>{clearTimeout(killTimer);reject(new Error('Owned MCP process did not exit after shutdown'));},7000);const done=()=>{clearTimeout(killTimer);clearTimeout(limit);resolve();};if(this.exit!==undefined)done();else this.child.once('exit',done);});this.save();}
}
