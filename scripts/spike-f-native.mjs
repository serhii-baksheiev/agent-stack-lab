...
for(const provider of ['claude','codex'])for(const order of ['rig-first','native-first']) {
 ...
 const install=()=>native('plugin-install',['plugin',provider==='codex'?'add':'install','lab-neutral@personal',...(provider==='claude'?['--scope','project']:[]),'--json']);
