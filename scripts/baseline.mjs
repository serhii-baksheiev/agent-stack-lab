...
assert(!existsSync(cwd),'fresh baseline-090 directory required');
...
assert.equal(init.exitCode,0);assert(after.files['.claude/.rig-manifest.json']);assert.equal(after.gitStatus.exitCode,0);assert.equal(after.gitDiff.exitCode,0);
