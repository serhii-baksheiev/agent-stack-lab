...
export function run(command, args, cwd, log, childEnvironment = {}) {
  const start = Date.now();
  // Callers supply only fixed laboratory arguments. Never pass credentials here.
  const r = spawnSync(command, args, { cwd, encoding:'utf8', maxBuffer:16*1024*1024, shell: process.platform === 'win32' && /^(npm|npx)$/.test(command), timeout:180000, env:{...process.env, NO_COLOR:'1', GIT_TERMINAL_PROMPT:'0',...childEnvironment} });
...
export function snapshot(cwd, output, childEnvironment = {}) {
  const files = {};
  ...
  walk(cwd);
  const gitStatus=run('git',['status','--short','--untracked-files=all'],cwd,undefined,childEnvironment),gitDiff=run('git',['diff','--no-ext-diff','HEAD'],cwd,undefined,childEnvironment);
